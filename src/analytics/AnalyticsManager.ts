...
export class AnalyticsManager {
 ...
 readonly sessionId=uuid(); private queue:AnalyticsEvent[]=[]; private activeMs=0; private lastActivity=performance.now(); private started=false; private currentStage:string|null=null; private diagnosticLogged=false; private flushDiagnosticLogged=false;
 ...
 initialize(){if(this.started)return;this.started=true;const host=typeof location!=='undefined'?location.hostname:'';if(!this.diagnosticLogged){this.diagnosticLogged=true;console.info('[AUDIO/ANALYTICS] runtime config',{endpointPresent:Boolean(this.endpoint),endpointHost:this.endpoint?(()=>{try{return new URL(this.endpoint).host}catch{return 'invalid'}})():'',keyPresent:Boolean(this.key),isTest:host==='localhost'||host==='127.0.0.1',queueLength:this.queue.length});}this.track('session_start',{startedAt:new Date().toISOString()});if(typeof window!=='undefined'){window.addEventListener('pagehide',()=>{this.endSession();});window.addEventListener('visibilitychange',()=>{if(document.hidden)this.lastActivity=performance.now();});}void this.flush();}
 ...
 async flush(){if(!this.endpoint||!this.key||!this.queue.length){if(!this.endpoint||!this.key){if(!this.flushDiagnosticLogged){this.flushDiagnosticLogged=true;console.warn('[ANALYTICS] disabled: missing client config',{endpointPresent:Boolean(this.endpoint),keyPresent:Boolean(this.key),queueLength:this.queue.length});}}return;}const batch=this.queue.slice();try{const res=await fetch(`${this.endpoint}/rest/v1/analytics_events`,{method:'POST',headers:{apikey:this.key,Authorization:`Bearer ${this.key}`, 'Content-Type':'application/json',Prefer:'return=minimal'},body:JSON.stringify(batch)});if(!this.flushDiagnosticLogged){this.flushDiagnosticLogged=true;console.info('[ANALYTICS] insert response',{status:res.status,ok:res.ok,batchSize:batch.length,isTest:batch[0]?.isTest??null});}if(res.ok||res.status===409){this.queue.splice(0,batch.length);this.persist();}else{console.warn('[ANALYTICS] insert rejected',{status:res.status,batchSize:batch.length});}}catch(error){if(!this.flushDiagnosticLogged){this.flushDiagnosticLogged=true;console.warn('[ANALYTICS] insert failed',{errorName:error instanceof Error?error.name:'unknown',queueLength:this.queue.length});}/* Analytics must never interrupt gameplay. */}}
}
